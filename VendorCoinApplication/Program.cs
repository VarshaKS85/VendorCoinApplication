...
using VendorCoinApplication.Repos;
using VendorCoinApplication.Services;
...
namespace VendorCoinApplication
{
    class Program
    {
        ...
        {
            var vm = new VendingMachine(new CoinsService(), new ProductsService(new ProductRepos(), new ProductInventoryRepos()));
        ...
        static void WriteVendingResponseToScreen(MachineResponse response)
        {
        ...
        static void WriteItemChangeToScreen(IEnumerable<ChangeItem> change)
        {
            Console.WriteLine("\nChange Returned\n");
